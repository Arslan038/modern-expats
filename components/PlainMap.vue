  ...
  <section ref="sectionEl" class="bg-background py-16 relative overflow-hidden">
    ...
    <div class="absolute top-10 left-10 w-24 h-24 opacity-5">
      <svg viewBox="0 0 100 100" class="text-primary">
        <path fill="currentColor" d="M50,5A45,45,0,1,1,5,50,45.05,45.05,0,0,1,50,5Zm0,80A35,35,0,1,0,15,50,35,35,0,0,0,50,85Zm0-60A25,25,0,1,1,25,50,25,25,0,0,1,50,25Zm0,40A15,15,0,1,0,35,50,15,15,0,0,0,50,65Z"/>
      </svg>
    </div>
    ...
    <div class="container mx-auto px-6 relative z-10">
      <div class="text-center mb-12">
        <h2 class="font-display font-bold text-4xl md:text-6xl text-foreground mb-6">
          Our <span class="text-primary relative">Global Footprints
            <span class="absolute -bottom-1 left-0 w-full h-3 bg-primary/20 -z-10 rounded-full"></span>
          </span>
        </h2>
        <p class="font-body text-lg text-muted-foreground max-w-2xl mx-auto">
          We've traveled across the globe, leaving memories in every corner.  
          Explore the map below to see where our journey has taken us.
        </p>
      </div>

      <!-- Map Container -->
      <div class="relative rounded-2xl overflow-hidden shadow-2xl border border-border/20">
        <div ref="mapEl" class="w-full h-[500px] md:h-[600px]"></div>
        
        <!-- Custom Map Controls -->
        <div class="absolute top-4 right-4 flex flex-col gap-2 bg-card/80 backdrop-blur-sm rounded-lg p-1.5 shadow-lg border border-border/10">
          <button @click="zoomIn" class="w-8 h-8 flex items-center justify-center rounded-md hover:bg-accent/10 transition-colors">
            <svg class="w-5 h-5 text-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
            </svg>
          </button>
          <button @click="zoomOut" class="w-8 h-8 flex items-center justify-center rounded-md hover:bg-accent/10 transition-colors">
            <svg class="w-5 h-5 text-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Map Legend -->
        <div class="absolute bottom-4 left-4 bg-card/80 backdrop-blur-sm rounded-lg p-3 shadow-lg border border-border/10">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mt-12">
        <div v-for="(stat, index) in stats" :key="index" class="text-center p-6 rounded-xl bg-card border border-border/10 shadow-sm">
